...
VitaForge is a modern fitness and gym management platform designed to connect gym members, trainers, and administrators through a unified web application.

The client application provides role-based dashboards, fitness class management, trainer applications, community/forum features, authentication, and a responsive user experience.
...
* Trainer-specific dashboard access
...
```bash
git clone https://github.com/FahmidaAkterShimu/vitaforge-client.git
```
...
VitaForge uses three primary roles:

| Role    | Access                                           |
| ------- | ------------------------------------------------ |
| User    | Classes, community, profile and fitness features |
| Trainer | Trainer dashboard and class management           |
| Admin   | Full platform management                         |

New users are registered with the `user` role by default.

Users can apply to become trainers. After an administrator approves the application, their role can be changed to `trainer`.

---

## 🔒 Protected Routes

Dashboard routes are protected according to the authenticated user's role.
...
The frontend communicates with the VitaForge Express.js backend through:
...
The theme is managed using `next-themes` and follows the application's design system.
...
BETTER_AUTH_URL=https://your-client-domain.com
...
```text
https://github.com/FahmidaAkterShimu/vitaforge-server
```
...
## 2. `vitaforge-server` — Backend README
